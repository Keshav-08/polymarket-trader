...
import {
  Zap, ArrowLeft, Activity, RefreshCw, ChevronUp, ChevronDown, Filter
} from "lucide-react";
...
export default function HistoryPage() {
  ...
  const sorted = [...history].sort((a, b) => {
    let valA: any;
    let valB: any;
    if (sortField === "triggered_at") {
      valA = new Date(a.triggered_at).getTime();
      valB = new Date(b.triggered_at).getTime();
    } else if (sortField === "ticker") {
      valA = a.ticker; valB = b.ticker;
    } else if (sortField === "total_value") {
      valA = a.total_value || 0; valB = b.total_value || 0;
    } else if (sortField === "probability") {
      valA = a.probability; valB = b.probability;
    }
  ...
  function SortIcon({ field }: { field: SortField }) {
    ...
    return sortDir === "desc"
      ? <ChevronDown size={12} className="text-[#00C48C]" />
      : <ChevronUp size={12} className="text-[#00C48C]" />;
  }
        ...
        <div className="max-w-7xl mx-auto flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-[#8B949E] text-xs">Trade History</p>
            </div>
          </div>
          <div className="flex items-center gap-3">
            <Link href="/rules" className="text-[#8B949E] hover:text-white text-xs transition-colors border border-[#30363D] hover:border-[#00C48C] px-3 py-1.5 rounded-lg">
              Rules
            </Link>
      ...
      <main className="max-w-7xl mx-auto px-6 py-8">
        ...
        {summary && (
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
            ...
            <StatCard
              ...
              value={`$${summary.total_value_traded.toLocaleString("en-US", { minimumFractionDigits: 2 })}`}
              sub="paper money"
            />
          </div>
        )}
        ...
        <div className="bg-[#161B22] border border-[#30363D] rounded-xl p-4 mb-6 flex flex-wrap items-center gap-4">
          ...
          <button
            ...
            className="ml-auto flex items-center gap-1.5 text-[#8B949E] hover:text-white text-xs transition-colors">
        ...
        <div className="bg-[#161B22] border border-[#30363D] rounded-xl overflow-hidden">
          <div className="grid grid-cols-[1fr_90px_80px_90px_90px_90px_120px] gap-4 px-4 py-3 border-b border-[#30363D] text-xs text-[#8B949E] font-mono">
            <button className="text-left flex items-center gap-1 hover:text-white transition-colors"
              onClick={() => toggleSort("triggered_at")}>
              MARKET / RULE <SortIcon field="triggered_at" />
            </button>
            <button className="text-left flex items-center gap-1 hover:text-white transition-colors"
          ...
          {loading ? (
            <div className="space-y-px">
              {[...Array(5)].map((_, i) => (
                <div key={i} className="h-16 bg-[#0D1117]/40 animate-pulse border-b border-[#30363D]" />
              ))}
            </div>
          ) : sorted.length === 0 ? (
            <div className="text-center py-16">
              ...
              <Link href="/rules" className="inline-block mt-4 text-[#00C48C] text-xs hover:underline">
                Create a rule →
              </Link>
            </div>
          ) : (
            <div>
              {sorted.map((trade) => (
                <div
                  key={trade.id}
                  className={`grid grid-cols-[1fr_90px_80px_90px_90px_90px_120px] gap-4 px-4 py-4 border-b border-[#30363D]/50 last:border-0 hover:bg-[#0D1117]/40 transition-colors ${
                    trade.execution_error ? "opacity-60" : ""
                  }`}>

                  <div className="min-w-0">
                    <p className="text-white text-sm truncate leading-snug">
                      {trade.market_question || "—"}
                    </p>
                    <p className="text-[#8B949E] text-xs mt-0.5 truncate">
                      Rule: {trade.rule_name}
                    </p>
                    {trade.execution_error && (
                      <p className="text-red-400 text-xs mt-0.5 truncate">
                        Error: {trade.execution_error}
                      </p>
                    )}
                  </div>

                  <div className="font-mono font-bold text-white text-sm self-center">
                    {trade.ticker}
                  </div>
                  ...
                  <div className="font-mono text-white text-sm self-center">
                    ...
                    {trade.total_value ? (
                      <div>
                        <p className="font-mono text-white text-sm">
                          ${trade.total_value.toLocaleString("en-US", { minimumFractionDigits: 2 })}
                        </p>
                        {trade.filled_price && (
                  ...
                  <div className="self-center">
                    <p className={`font-mono text-sm font-bold ${
                      trade.probability >= 70 ? "text-[#00C48C]" :
                      trade.probability >= 40 ? "text-yellow-400" : "text-red-400"
                    }`}>
                      {trade.probability?.toFixed(1)}%
                    </p>
                    {trade.shift !== null && trade.shift !== undefined && (
                      <p className={`text-xs font-mono ${
                        trade.shift > 0 ? "text-[#00C48C]" :
                        trade.shift < 0 ? "text-red-400" : "text-[#8B949E]"
                      }`}>
                  ...
                  <div className="self-center">
                    <p className="text-[#8B949E] text-xs font-mono">
                      {new Date(trade.triggered_at).toLocaleDateString()}
                    </p>
                    <p className="text-[#8B949E] text-xs font-mono">
                      {new Date(trade.triggered_at).toLocaleTimeString()}
                    </p>
                  </div>
                </div>
              ))}
        ...
        {sorted.length > 0 && (
          <p className="text-[#8B949E] text-xs text-center mt-4 font-mono">
            {sorted.length} trade{sorted.length > 1 ? "s" : ""} shown
          </p>
        )}
      </main>
    </div>
