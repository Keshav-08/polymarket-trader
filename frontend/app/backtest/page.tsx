...
import {
  Zap, ArrowLeft, Play, Activity, AlertTriangle
} from "lucide-react";
...
interface BacktestResult {
  ...
  trades: Trade[];
  meta: {
    keyword: string; ticker: string; threshold: number;
    action: string; start: string; end: string;
    market_question: string | null;
    prob_points: number; price_points: number;
  };
}
    ...
    <div className="bg-[#161B22] border border-[#30363D] rounded-lg p-3 text-xs shadow-xl">
      ...
      {d?.trade && (
        <div className="mt-1 pt-1 border-t border-[#30363D]">
          <p className={`font-mono font-bold ${d.trade.type.includes("BUY") ? "text-[#00C48C]" : "text-red-400"}`}>
            {d.trade.type} @ ${d.trade.price?.toFixed(2)}
          </p>
          {d.trade.pnl !== undefined && (
            <p className={`font-mono ${d.trade.pnl >= 0 ? "text-[#00C48C]" : "text-red-400"}`}>
              P&L: {d.trade.pnl >= 0 ? "+" : ""}${d.trade.pnl?.toFixed(2)}
            </p>
          )}
        </div>
      )}
  ...
  return (
    <g>
      ...
      <text x={cx} y={cy + 1} textAnchor="middle" dominantBaseline="middle"
        fontSize={8} fontWeight="bold" fill="#0D1117">
        {isBuy ? "B" : "S"}
      </text>
    </g>
  );
...
export default function BacktestPage() {
  ...
  async function runBacktest() {
    ...
    try {
      ...
      const r = await fetch(`${API}/api/backtest?${params}`);
      if (!r.ok) {
        ...
        <div className="max-w-7xl mx-auto flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-[#8B949E] text-xs">Backtesting</p>
            </div>
          </div>
          <div className="flex items-center gap-3">
              ...
              {keyword && ticker && (
                <div className="bg-[#0D1117] border border-[#30363D] rounded-lg px-4 py-3">
                  ...
                  <p className="text-sm text-white">
                    ...
                    {" → "}
                    <span className={action === "buy" ? "text-[#00C48C]" : "text-red-400"}>
                      {action.toUpperCase()}
                    </span>{" "}
              ...
              <>
                {result.meta.market_question && (
                  <div className="bg-[#161B22] border border-[#30363D] rounded-xl px-5 py-3">
                    ...
                    <p className="text-[#8B949E] text-xs mt-1 font-mono">
                      {result.meta.prob_points} probability points · {result.meta.price_points} price bars
                    </p>
                  </div>
                )}
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                  <SummaryCard
                    ...
                    sub={`${result.summary.total_trades} trades`}
                  ...
                  <SummaryCard
                    ...
                    positive={result.summary.win_rate >= 50}
                  />
                  <SummaryCard
                    label="Best Trade"
                    value={`+$${result.summary.best_trade.toFixed(2)}`}
                    positive={true}
                  />
                  <SummaryCard
                    label="Worst Trade"
                    value={`$${result.summary.worst_trade.toFixed(2)}`}
                    positive={result.summary.worst_trade >= 0}
                  />
                </div>
                ...
                <div className="bg-[#161B22] border border-[#30363D] rounded-xl p-5">
                  <div className="flex items-center justify-between mb-4">
                    <h3 className="text-white font-semibold">
                      {result.meta.ticker} Price + Trade Markers
                    </h3>
                    <div className="flex items-center gap-4 text-xs text-[#8B949E]">
                      <span className="flex items-center gap-1">
                        <span className="w-3 h-3 rounded-full bg-[#00C48C] inline-block" /> Buy
                      </span>
                      <span className="flex items-center gap-1">
                  ...
                  {result.chart_data.length === 0 ? (
                    ...
                    <ResponsiveContainer width="100%" height={320}>
                      <ComposedChart data={result.chart_data} margin={{ top: 10, right: 10, left: 0, bottom: 0 }}>
                        ...
                        <YAxis yAxisId="price" tick={{ fill: "#8B949E", fontSize: 10 }}
                          tickLine={false} axisLine={false} tickFormatter={v => `$${v}`} domain={["auto", "auto"]} />
                        <YAxis yAxisId="prob" orientation="right"
                          tick={{ fill: "#8B949E", fontSize: 10 }}
                          tickLine={false} axisLine={false}
                          tickFormatter={v => `${v}%`} domain={[0, 100]} />
                        <Tooltip content={<CustomTooltip />} />
                        <ReferenceLine yAxisId="prob" y={result.meta.threshold}
                          stroke="#F59E0B" strokeDasharray="4 4"
                          label={{ value: `${result.meta.threshold}%`, fill: "#F59E0B", fontSize: 10, position: "right" }} />
                        <Area yAxisId="prob" type="monotone" dataKey="probability"
                          stroke="#F59E0B" strokeWidth={1.5} fill="#F59E0B" fillOpacity={0.06} dot={false} />
                        <Line yAxisId="price" type="monotone" dataKey="price"
                          stroke="#00C48C" strokeWidth={2}
                          dot={<CustomDot />} activeDot={{ r: 4, fill: "#00C48C" }} />
                      </ComposedChart>
                    </ResponsiveContainer>
                  )}
                  <p className="text-[#8B949E] text-xs text-center mt-2">
                    Green = {result.meta.ticker} price · Yellow = probability · Dashed = threshold · B/S = trade markers
                  </p>
                </div>
                        ...
                        <div key={i} className="flex items-center justify-between py-2 border-b border-[#30363D]/50 last:border-0">
                          <div className="flex items-center gap-3">
                            <span className={`text-xs font-mono font-bold px-2 py-0.5 rounded ${
                              trade.type.includes("BUY") ? "bg-[#00C48C]/20 text-[#00C48C]" : "bg-red-500/20 text-red-400"
                            }`}>
                              {trade.type}
                            </span>
